...
# For demonstration, we'll mock the MobileNetV4 implementation
# Replace this with your actual build_models.MobileNetV4
class MobileNetV4(nn.Module):
    def __init__(self, model_name='MobileNetV4ConvSmall'):
        super(MobileNetV4, self).__init__()
        # Mock architecture (replace with actual MobileNetV4 layers)
        ...
        self.layer4 = nn.Sequential(
            nn.Conv2d(96, 1280, 3, stride=2, padding=1),
            nn.BatchNorm2d(1280),
            nn.ReLU()
        )
...
# Modified ConvNeXt for teacher model with stage-wise feature extraction
class ConvNeXt(nn.Module):
    def __init__(self, num_classes=4):
        ...
        self.stage1 = nn.Sequential(*base.features[:2])  # 96 channels
        self.stage2 = nn.Sequential(*base.features[2:4])  # 192 channels
        self.stage3 = nn.Sequential(*base.features[4:6])  # 384 channels
        self.stage4 = nn.Sequential(*base.features[6:8])  # 768 channels
        self.stage5 = nn.Sequential(base.avgpool, nn.Flatten())  # 768 channels
    ...
    def forward(self, x, return_features=False):
        ...
        for i in range(num_frames):
            ...
            feat = self.stage1(frame)
            features.append(feat.mean([2, 3]))  # Stage 1: [batch_size, 96]
            feat = self.stage2(feat)
            features.append(feat.mean([2, 3]))  # Stage 2: [batch_size, 192]
            feat = self.stage3(feat)
            features.append(feat.mean([2, 3]))  # Stage 3: [batch_size, 384]
            feat = self.stage4(feat)
            features.append(feat.mean([2, 3]))  # Stage 4: [batch_size, 768]
            feat = self.stage5(feat)
            features.append(feat)  # Stage 5: [batch_size, 768]
        for i in range(5):
            features[i] = torch.stack(features[i::5], dim=1).mean(dim=1)
        if return_features:
            return features
...
class MultimodalWorkoutClassifier(nn.Module):
    def __init__(self, num_classes=4, modal_num=3):
        ...
        base = build_models(model_name='MobileNetV4ConvSmall')
        self.layers = base
        ...
        if modal_num == 3:
            self.flow_model = build_models(model_name='MobileNetV4ConvSmall')
            self.mask_model = build_models(model_name='MobileNetV4ConvSmall')
        ...
        self.channel_counts = [32, 32, 64, 96, 1280]  # Adjust based on actual MobileNetV4 architecture
    ...
    def forward(self, rgb, flow=None, mask=None, return_features=False):
        ...
        feat = x.mean([3, 4]).mean(dim=1)  # [batch_size, channels]
        ...
        if self.modal_num == 3:
            ...
            x_flow = x_flow.view(batch_size, num_frames, -1)
            flow_features = x_flow.mean(dim=1)
            ...
            x_mask = x_mask.view(batch_size, num_frames, -1)
            mask_features = x_mask.mean(dim=1)
...
# Modified LayerWiseVkD
class LayerWiseVkD(nn.Module):
    def __init__(self, teacher, student, feature_dims=[96, 192, 384, 768, 768], modal_num=3):
        super(LayerWiseVkD, self).__init__()
        self.teacher = teacher
        self.student = student
        self.modal_num = modal_num
        student_channel_counts = [32, 32, 64, 96, 1280]  # Adjust based on actual MobileNetV4 architecture
        ...
        if train_student:
            ...
            for mod_idx in range(self.modal_num):
                ...
                for stage_idx in range(5):
                    ...
                    expected_channels = self.feature_adapters[stage_idx][mod_idx].in_features
                    if feat.size(1) != expected_channels:
                        print(f"Feature dimension mismatch at stage {stage_idx}, mod {mod_idx}: expected {expected_channels}, got {feat.size(1)}")
                        feat = nn.AdaptiveAvgPool1d(expected_channels)(feat.unsqueeze(-1)).squeeze(-1)
            ...
            for mod_idx in range(self.modal_num):
                ...
                expected_channels = self.feature_adapters[-1][mod_idx].in_features
                if feat.size(1) != expected_channels:
                    feat = nn.AdaptiveAvgPool1d(expected_channels)(feat.unsqueeze(-1)).squeeze(-1)
...
def feature_loss(student_features, teacher_features):
    ...
    for mod_features in student_features:
        for stage_idx, (s, t) in enumerate(zip(mod_features, teacher_features)):
            total_loss += F.mse_loss(s, t)
    return total_loss / (len(student_features) * num_stages)

# Training function for LayerWiseVkD
def train_vkd(model, train_loader, optimizer, epoch, device, modal_num):
    model.train()
    total_loss = 0
    for rgb, flow, mask, labels in tqdm(train_loader):
        rgb, flow, mask, labels = rgb.to(device), flow.to(device) if flow is not None else None, mask.to(device) if mask is not None else None, labels.to(device)
        optimizer.zero_grad()
        if modal_num == 3:
...
def plot_results(train_losses, val_losses, val_accuracies, val_f1_scores, val_precisions, val_recalls, modal_num):
    suffix = f'mobilenetv4_single_layerwise_vkd' if modal_num == 1 else f'mobilenetv4_multimodal_layerwise_vkd'
...
def plot_confusion_matrix(y_true, y_pred, class_names, normalize=True, figsize=(10, 8), cmap='Blues', suffix='', modal_num=3):
    suffix = f'_layerwise_vkd_{suffix}_mobilenetv4_single' if modal_num == 1 else f'_layerwise_vkd_{suffix}_mobilenetv4_multimodal'
...
# Initialize models
teacher = ConvNeXt(num_classes=num_classes).to(device)
student = MultimodalWorkoutClassifier(num_classes=num_classes, modal_num=modal_num).to(device)
teacher.eval()

# Initialize LayerWiseVkD model
model = LayerWiseVkD(teacher, student, feature_dims=[96, 192, 384, 768, 768], modal_num=modal_num).to(device)
