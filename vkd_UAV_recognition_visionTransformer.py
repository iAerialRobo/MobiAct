...
# Modified Vision Transformer for teacher model with stage-wise feature extraction
class VisionTransformer(nn.Module):
    def __init__(self, num_classes=4, image_size=128, patch_size=16, num_layers=12, hidden_dim=768):
        super(VisionTransformer, self).__init__()
        # Load pre-trained ViT model
        self.vit = models.vit_b_16(weights='IMAGENET1K_V1')
        # Override image_size to 128
        self.vit.image_size = image_size
        # Modify patch embedding for 128x128 input
        ...
        orig_num_patches = (224 // patch_size) ** 2
        orig_pos_embed = self.vit.encoder.pos_embedding  # Shape: [1, 1+orig_num_patches, hidden_dim]
        new_pos_embed = torch.nn.functional.interpolate(
            orig_pos_embed[:, 1:, :].permute(0, 2, 1),  # Remove CLS token, shape: [1, hidden_dim, orig_num_patches]
            size=num_patches,
            mode='linear'
        ).permute(0, 2, 1)  # Shape: [1, num_patches, hidden_dim]
        cls_token = orig_pos_embed[:, :1, :]  # Keep CLS token embedding
        self.vit.encoder.pos_embedding = nn.Parameter(
            torch.cat([cls_token, new_pos_embed], dim=1)
        )
    ...
    def forward(self, x, return_features=False):
        ...
        x = self.vit._process_input(x)  # Shape: (batch_size * num_frames, c, h, w)
        n = x.shape[0]
        batch_class_token = self.vit.class_token.expand(n, -1, -1)
        x = torch.cat([batch_class_token, x], dim=1)
        x = self.vit.encoder.pos_embedding + x
        features.append(x[:, 0].view(batch_size, num_frames, -1).mean(dim=1))  # Stage 1: Patch embeddings

        # Process through transformer layers
        for i, layer in enumerate(self.vit.encoder.layers):
            x = layer(x)
            if i in [2, 5, 8, 11]:  # Extract features at specific layers
                features.append(x[:, 0].view(batch_size, num_frames, -1).mean(dim=1))  # Stage 2-5: Transformer layers
...
class LayerWiseVkD(nn.Module):
    def __init__(self, teacher, student, feature_dims=[768, 768, 768, 768, 768], modal_num=3):
...
if balanced_train:
    train_loader = DataLoader(dataset=train_dataset, shuffle=False, batch_size=20, sampler=sampler)
else:
...
# Initialize models
teacher = VisionTransformer(num_classes=num_classes, image_size=128).to(device)
student = MultimodalWorkoutClassifier(num_classes=num_classes, modal_num=modal_num).to(device)
teacher.eval()

# Initialize LayerWiseVkD model
model = LayerWiseVkD(teacher, student, feature_dims=[768, 768, 768, 768, 768], modal_num=modal_num).to(device)
