...
class ResNet18(nn.Module):
    ...
    def forward(self, x, return_features=False):
        ...
        features.append(x)  # Final features: 512 channels
        if return_features:
            return features
        x = self.classifier(x)
        return x
...
# Modified LayerWiseVkD to use only C2 (stage 2) features and KL loss
class LayerWiseVkD(nn.Module):
    def __init__(self, teacher, student, feature_dims=[64], modal_num=3):
        super(LayerWiseVkD, self).__init__()
        self.teacher = teacher
        self.student = student
        self.modal_num = modal_num
        student_channel_counts = [32]  # Only stage 2 (C2)
        self.feature_adapters = nn.ModuleList([
            nn.ModuleList([
                nn.Linear(student_channel_counts[0], feature_dims[0])
                for mod_idx in range(modal_num)
            ])
        ])
        self.projections = nn.ModuleList([
            OrthogonalProjection(feature_dims[0], feature_dims[0])
        ])
    ...
    def forward(self, rgb, flow=None, mask=None, train_student=True):
        ...
        with torch.no_grad():
            ...
            teacher_features = [(teacher_features[1] - teacher_features[1].mean(dim=0)) / (teacher_features[1].std(dim=0) + 1e-6)]  # Only C2

        if train_student:
            student_features = self.student(rgb, flow, mask, return_features=True)
            adapted_features = []
            for mod_idx in range(self.modal_num):
                feat = student_features[mod_idx][1]  # Only C2 (stage 2)
                feat = self.feature_adapters[0][mod_idx](feat)
                feat = self.projections[0](feat)
...
# Custom feature loss for C2 only
def feature_loss(student_features, teacher_features):
    total_loss = 0
    for mod_features in student_features:
        total_loss += F.mse_loss(mod_features[0], teacher_features[0])  # Only C2
...
def train_vkd(model, train_loader, optimizer, epoch, device, modal_num):
    ...
    for rgb, flow, mask, labels in tqdm(train_loader):
        ...
        optimizer.zero_grad()
        with torch.no_grad():
            teacher_logits = model.teacher(rgb)
        if modal_num == 3:
            student_logits, student_features, teacher_features = model(rgb, flow, mask)
        else:
            student_logits, student_features, teacher_features = model(rgb)
...
def plot_results(train_losses, val_losses, val_accuracies, val_f1_scores, val_precisions, val_recalls, modal_num):
    suffix = f'mobilenetv4_single_c2_kl' if modal_num == 1 else f'mobilenetv4_multimodal_c2_kl'
...
def plot_confusion_matrix(y_true, y_pred, class_names, normalize=True, figsize=(10, 8), cmap='Blues', suffix='', modal_num=3):
    suffix = f'_c2_kl_{suffix}_mobilenetv4_single' if modal_num == 1 else f'_c2_kl_{suffix}_mobilenetv4_multimodal'
...
# Initialize LayerWiseVkD model with only C2
model = LayerWiseVkD(teacher, student, feature_dims=[64], modal_num=modal_num).to(device)
