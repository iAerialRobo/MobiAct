...
class ResNet18(nn.Module):
    ...
    def forward(self, x, return_features=False):
        ...
        if return_features:
            return features[:2]  # Only return C2, C3
        x = self.classifier(x)
        return x
...
# Modified LayerWiseVkD using C2, C3 with KL loss
class LayerWiseVkD(nn.Module):
    def __init__(self, teacher, student, feature_dims=[64, 128], modal_num=3):
        super(LayerWiseVkD, self).__init__()
        self.teacher = teacher
        self.student = student
        self.modal_num = modal_num
        student_channel_counts = [32, 64]  # C2, C3 from MobileNetV4ConvSmall
    ...
    def forward(self, rgb, flow=None, mask=None, train_student=True):
        ...
        if train_student:
            ...
            for mod_idx in range(self.modal_num):
                mod_adapted = []
                for stage_idx in range(2):  # Only C2, C3
...
# KL Divergence loss for features
def feature_kl_loss(student_features, teacher_features, temperature=1.0):
    total_loss = 0
    for mod_features in student_features:
        for s, t in zip(mod_features, teacher_features):
            s = F.softmax(s / temperature, dim=1)
            t = F.softmax(t / temperature, dim=1)
            total_loss += F.kl_div(s.log(), t, reduction='batchmean')
...
def train_vkd(model, train_loader, optimizer, epoch, device, modal_num):
    ...
    for rgb, flow, mask, labels in tqdm(train_loader):
        ...
        feat_kl_loss = feature_kl_loss(student_features, teacher_features, temperature=2.0)
        alpha = 0.5
        loss = cls_loss + alpha * feat_kl_loss
        loss.backward()
        optimizer.step()
        total_loss += loss.item()
    avg_loss = total_loss / len(train_loader)
    print(f'Epoch {epoch}: Train Loss: {avg_loss:.4f} (Cls: {cls_loss.item():.4f}, Feat KL: {feat_kl_loss.item():.4f})')
    return avg_loss
...
def plot_results(train_losses, val_losses, val_accuracies, val_f1_scores, val_precisions, val_recalls, modal_num):
    suffix = f'mobilenetv4_single_layerwise_vkd_c2_c3_kl' if modal_num == 1 else f'mobilenetv4_multimodal_layerwise_vkd_c2_c3_kl'
...
def plot_confusion_matrix(y_true, y_pred, class_names, normalize=True, figsize=(10, 8), cmap='Blues', suffix='', modal_num=3):
    suffix = f'_layerwise_vkd_c2_c3_kl_{suffix}_mobilenetv4_single' if modal_num == 1 else f'_layerwise_vkd_c2_c3_kl_{suffix}_mobilenetv4_multimodal'
...
model = LayerWiseVkD(teacher, student, feature_dims=[64, 128], modal_num=modal_num).to(device)
