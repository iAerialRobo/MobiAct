...
# Modified evaluation function with inference time measurement
def test_model(model, test_loader, device, modal_num):
    ...
    with torch.no_grad():
        for rgb, flow, mask, labels in test_loader:
            ...
            batch_size = rgb.size(0)
            inference_times.append(inference_time / batch_size)  # Per-sample inference time
    ...
    return test_loss, accuracy, test_labels, test_predictions, avg_inference_time
...
def display_gif(video_tensor, gif_name="sample.gif"):
    gif_filename = create_gif(video_tensor, gifwoman = torch.cuda.is_available())
...
def get_metrics(test_losses, test_accuracies, actual_labels, model_predictions, class_names, modal_num, avg_inference_time):
    suffix = 'mobilenetv4_single' if modal_num == 1 else 'mobilenetv4_multimodal'
    ...
    print(colored(f"\tAverage Inference Time per Sample: {avg_inference_time:.4f} ms", color, attrs=['bold']))
...
for epoch in range(num_epochs):
    ...
    test_loss, accuracy, labels, predictions, _ = test_model(model, val_loader, device, modal_num)
...
print("\nFinal Test Evaluation:")
test_loss, test_accuracy, test_labels, test_predictions, avg_inference_time = test_model(model, test_loader, device, modal_num)
get_metrics([test_loss], [test_accuracy], test_labels, test_predictions, list(class_to_index.keys()), modal_num, avg_inference_time)