...
modal_num = 1  # Change to 1 for single-modality
...
class MultimodalWorkoutClassifier(nn.Module):
    def __init__(self, num_classes=4, modal_num=3):
        ...
        # Add attention modules for each stage
        self.rgb_attentions = nn.ModuleList([
            CBAM(channels) for channels in self.channel_counts
        ])

        if modal_num == 3:
            self.flow_attentions = nn.ModuleList([
                CBAM(channels) for channels in self.channel_counts
            ])
    ...
    def forward(self, rgb, flow=None, mask=None, return_features=False, return_contrastive=False):
        ...
        x_reshaped = self.rgb_attentions[0](x_reshaped)
        x = x_reshaped.view(batch_size, num_frames, x.size(2), x.size(3), x.size(4))
