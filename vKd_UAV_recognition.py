...
class MultimodalWorkoutClassifier(nn.Module):
    def __init__(self, num_classes=4, modal_num=3):
        ...
        self.layers = base  # The entire MobileNetV4 model
        # Define stages based on MobileNetV4ConvSmall architecture
        ...
        # Correct channel counts based on MobileNetV4ConvSmall
    ...
    def forward(self, rgb, flow=None, mask=None, return_features=False):
        ...
        rgb = rgb.view(-1, c, h, w)  # Shape: (batch_size * num_frames, c, h, w)
        features_rgb = []
        x = self.stage1(rgb)
        x = x.view(batch_size, num_frames, x.size(1), x.size(2), x.size(3))
        feat = x.mean([3, 4]).mean(dim=1)  # Shape: (batch_size, 32)
       # print(f"RGB Stage 1 feature shape: {feat.shape}")  # Debug
        features_rgb.append(feat)
        x = x.view(-1, x.size(2), x.size(3), x.size(4))
        x = self.stage2(x)
        x = x.view(batch_size, num_frames, x.size(1), x.size(2), x.size(3))
        feat = x.mean([3, 4]).mean(dim=1)  # Shape: (batch_size, 32)
      #  print(f"RGB Stage 2 feature shape: {feat.shape}")  # Debug
        features_rgb.append(feat)
        x = x.view(-1, x.size(2), x.size(3), x.size(4))
        x = self.stage3(x)
        x = x.view(batch_size, num_frames, x.size(1), x.size(2), x.size(3))
        feat = x.mean([3, 4]).mean(dim=1)  # Shape: (batch_size, 64)
     #   print(f"RGB Stage 3 feature shape: {feat.shape}")  # Debug
        features_rgb.append(feat)
        x = x.view(-1, x.size(2), x.size(3), x.size(4))
        x = self.stage4(x)
        x = x.view(batch_size, num_frames, x.size(1), x.size(2), x.size(3))
        feat = x.mean([3, 4]).mean(dim=1)  # Shape: (batch_size, 96)
      #  print(f"RGB Stage 4 feature shape: {feat.shape}")  # Debug
        features_rgb.append(feat)
        x = x.view(-1, x.size(2), x.size(3), x.size(4))
        x = self.stage5(x)
        x = x.view(batch_size, num_frames, x.size(1), x.size(2), x.size(3))
        rgb_features = x.mean([3, 4]).mean(dim=1)  # Shape: (batch_size, 1280)
       # print(f"RGB Stage 5 feature shape: {rgb_features.shape}")  # Debug
        ...
        if self.modal_num == 3:
            ...
            feat = x_flow.mean([3, 4]).mean(dim=1)  # Shape: (batch_size, 32)
           # print(f"Flow Stage 1 feature shape: {feat.shape}")  # Debug
            features_flow.append(feat)
            x_flow = x_flow.view(-1, x_flow.size(2), x_flow.size(3), x_flow.size(4))
            x_flow = self.flow_stage2(x_flow)
            x_flow = x_flow.view(batch_size, num_frames, x_flow.size(1), x_flow.size(2), x_flow.size(3))
            feat = x_flow.mean([3, 4]).mean(dim=1)  # Shape: (batch_size, 32)
          #  print(f"Flow Stage 2 feature shape: {feat.shape}")  # Debug
            features_flow.append(feat)
            x_flow = x_flow.view(-1, x_flow.size(2), x_flow.size(3), x_flow.size(4))
            x_flow = self.flow_stage3(x_flow)
            x_flow = x_flow.view(batch_size, num_frames, x_flow.size(1), x_flow.size(2), x_flow.size(3))
            feat = x_flow.mean([3, 4]).mean(dim=1)  # Shape: (batch_size, 64)
          #  print(f"Flow Stage 3 feature shape: {feat.shape}")  # Debug
            features_flow.append(feat)
            x_flow = x_flow.view(-1, x_flow.size(2), x_flow.size(3), x_flow.size(4))
            x_flow = self.flow_stage4(x_flow)
            x_flow = x_flow.view(batch_size, num_frames, x_flow.size(1), x_flow.size(2), x_flow.size(3))
            feat = x_flow.mean([3, 4]).mean(dim=1)  # Shape: (batch_size, 96)
          #  print(f"Flow Stage 4 feature shape: {feat.shape}")  # Debug
            features_flow.append(feat)
            x_flow = x_flow.view(-1, x_flow.size(2), x_flow.size(3), x_flow.size(4))
            x_flow = self.flow_stage5(x_flow)
            x_flow = x_flow.view(batch_size, num_frames, x_flow.size(1), x_flow.size(2), x_flow.size(3))
            flow_features = x_flow.mean([3, 4]).mean(dim=1)  # Shape: (batch_size, 1280)
          #  print(f"Flow Stage 5 feature shape: {flow_features.shape}")  # Debug
            ...
            feat = x_mask.mean([3, 4]).mean(dim=1)  # Shape: (batch_size, 32)
         #   print(f"Mask Stage 1 feature shape: {feat.shape}")  # Debug
            features_mask.append(feat)
            x_mask = x_mask.view(-1, x_mask.size(2), x_mask.size(3), x_mask.size(4))
            x_mask = self.mask_stage2(x_mask)
            x_mask = x_mask.view(batch_size, num_frames, x_mask.size(1), x_mask.size(2), x_mask.size(3))
            feat = x_mask.mean([3, 4]).mean(dim=1)  # Shape: (batch_size, 32)
          #  print(f"Mask Stage 2 feature shape: {feat.shape}")  # Debug
            features_mask.append(feat)
            x_mask = x_mask.view(-1, x_mask.size(2), x_mask.size(3), x_mask.size(4))
            x_mask = self.mask_stage3(x_mask)
            x_mask = x_mask.view(batch_size, num_frames, x_mask.size(1), x_mask.size(2), x_mask.size(3))
            feat = x_mask.mean([3, 4]).mean(dim=1)  # Shape: (batch_size, 64)
           # print(f"Mask Stage 3 feature shape: {feat.shape}")  # Debug
            features_mask.append(feat)
            x_mask = x_mask.view(-1, x_mask.size(2), x_mask.size(3), x_mask.size(4))
            x_mask = self.mask_stage4(x_mask)
            x_mask = x_mask.view(batch_size, num_frames, x_mask.size(1), x_mask.size(2), x_mask.size(3))
            feat = x_mask.mean([3, 4]).mean(dim=1)  # Shape: (batch_size, 96)
           # print(f"Mask Stage 4 feature shape: {feat.shape}")  # Debug
            features_mask.append(feat)
            x_mask = x_mask.view(-1, x_mask.size(2), x_mask.size(3), x_mask.size(4))
            x_mask = self.mask_stage5(x_mask)
            x_mask = x_mask.view(batch_size, num_frames, x_mask.size(1), x_mask.size(2), x_mask.size(3))
            mask_features = x_mask.mean([3, 4]).mean(dim=1)  # Shape: (batch_size, 1280)
           # print(f"Mask Stage 5 feature shape: {mask_features.shape}")  # Debug
...
class LayerWiseVkD(nn.Module):
    def __init__(self, teacher, student, feature_dims=[64, 64, 128, 256, 512], modal_num=3):
        ...
        self.modal_num = modal_num
        # Student channel counts from MobileNetV4ConvSmall
        student_channel_counts = [32, 32, 64, 96, 1280]
        # Initialize feature adapters: one adapter per stage per modality
        ...
        # Update student classifier to match teacher feature dimensions
        ...
        if train_student:
            ...
            for mod_idx in range(self.modal_num):
                ...
                for stage_idx in range(5):
                    ...
                    # print(f"Adapter input shape for modality {mod_idx}, stage {stage_idx}: {feat.shape}")
...
model = LayerWiseVkD(teacher, student, feature_dims=[64, 64, 128, 256, 512], modal_num=modal_num).to(device)
